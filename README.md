...
3. 调整每次加载数据的数量, 从20部电影调整为40部，快点不好吗;
4. 优化因不断加载数据, 从而导致 TV 版操作卡顿, 其他地方保持原版本不变;
5. 优化 "最近播放" 列表经常不显示的问题;
6. 修正从详情页返回列表页时，重新加载影片列表的问题;
7. 增加预设账号Guest，服务器地址, 方便没有后端的人可以直接用；
8. 显示视频源 ms & pt值（0-300ms越小越快，30-100pt越大越好），影片快慢参考；
9. 优化频繁验证服务器的逻辑；
